#This is a test file to run the genetic algorithm for the ridgepin optimization problem
import pygad
import numpy as np
import os
...
from randomFunctions import rename_file, save_tried_solutions, move_files_to_directory, create_run_directory, is_file_still_writing
from createRidgeFilter import create_ridgefilter
from dataAnalysis import plot_ridgepin_shape, total_fit_functions, fit_function_new, import_data_from_file, normalize_data, matplot_plotting
...
num_of_initial_population = 40 # Number of initial solutions
num_of_parents_mating = 4 # Number of parents that will mate and continue to the next generation
...
def fitness_func(ga_instance, 
    ...
    #fitness, fit_1, fit_2 = total_fit_functions(final_filename=final_filename, 
                                                # weight=fitvalue_weight, 
                                                # debug = False) 
    
    fitness_value = np.sum(solution)#fit_function_new(file_name=final_filename,debug=False, max_error=True, sqrt_fit_val=True)
    ...
    save_tried_solutions(solutions=solution, 
                         ...
                         file_name_data=created_directory,
...
def run_monte_carlo_simulation(coordinate_vector: list[float ],  
    ...
    # To - do make sure that the total width always remains the same.
...
def main():
    ...
    ga_instance = pygad.GA(
        ...
        keep_elitism=1,                 # Number of parents to keep in the next generation
        ...
        stop_criteria="saturate_15", # Stop criteria if the fitness value doesn't change for 5 generations
    )
